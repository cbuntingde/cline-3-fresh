...
exports.subscribeToFile = subscribeToFile;
const fs = __importStar(require("fs/promises"));
const fsSync = __importStar(require("fs"));
const watch_1 = require("../../../src/shared/proto/host/watch");
const host_grpc_handler_1 = require("../host-grpc-handler");
 ...
 */
async function subscribeToFile(request, responseStream, requestId) {
    ...
    try {
                ...
                if (eventType === "change") {
                    try {
                        ...
                        if (watcherInfo) {
                            ...
                            const eventType = watch_1.FileChangeEvent_ChangeType.CHANGED;
                            const now = Date.now();
                    ...
                    try {
                        ...
                        if (watcherInfo) {
                            // Check if this event should be debounced
                            const eventType = watch_1.FileChangeEvent_ChangeType.CREATED;
                            const now = Date.now();
                            const lastTime = watcherInfo.lastEventTime.get(eventType) || 0;
                            if (now - lastTime < DEBOUNCE_DELAY) {
                        ...
                        if (watcherInfo) {
                            // Check if this event should be debounced
                            const eventType = watch_1.FileChangeEvent_ChangeType.DELETED;
                            const now = Date.now();
                            const lastTime = watcherInfo.lastEventTime.get(eventType) || 0;
                            if (now - lastTime < DEBOUNCE_DELAY) {
        ...
        if (requestId) {
            (0, host_grpc_handler_1.getRequestRegistry)().registerRequest(requestId, cleanup, { type: "file_subscription", path: filePath }, responseStream);
        }
    }
    catch (error) {
        ...
        await responseStream({
            path: filePath,
            type: watch_1.FileChangeEvent_ChangeType.DELETED,
