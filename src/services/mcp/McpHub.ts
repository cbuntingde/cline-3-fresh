  ...
  "mcpServers": {
	...
	private async connectToServer(
		...
		try {
			...
			// Register MCP tools with intelligence registry
			if (this.toolIntelligenceRegistry && connection.server.tools) {
				for (const tool of connection.server.tools) {
					await this.toolIntelligenceRegistry.registerMcpTool(
						name,
						tool.name,
				...
				// Register the MCP server itself with intelligence metadata
				await this.toolIntelligenceRegistry.registerMcpServer(
					name,
					`MCP server: ${name}`,
					connection.server.tools.map(t => ({
						name: t.name,
						description: t.description,
						inputSchema: t.inputSchema
					})),
					connection.server.resources?.map(r => ({
						name: r.name,
