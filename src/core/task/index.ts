...
import { ShellDetector, CommandAdapter } from "@utils/shell-detection"
import {
	BrowserAction,
	BrowserActionResult,
	browserActions,
...
import { getAllExtensionState, getGlobalState } from "@core/storage/state"
...
import { tavilySearchService } from "@core/tavily/TavilySearchService"
...
export class Task {
	...
	constructor(
		...
		// Initialize Tavily search service if API key is available
		this.initializeTavilySearch().then(() => {
	...
	private async initializeTavilySearch(): Promise<void> {
		try {
			const tavilyApiKey = await getSecret(this.context, "tavilyApiKey")
			if (tavilyApiKey) {
				tavilySearchService.setApiKey(tavilyApiKey)
				console.log("Tavily search service initialized with API key")
			}
		} catch (error) {
	...
	// Check if the task is related to creating new apps
	private isAppCreationTask(task: string): boolean {
		const lowerTask = task.toLowerCase()

		// App creation keywords and patterns
		const appCreationPatterns = [
			/create (a|an|new) (\w+ )?app/gi,
			/build (a|an|new) (\w+ )?app/gi,
			/develop (a|an|new) (\w+ )?app/gi,
			/make (a|an|new) (\w+ )?app/gi,
			/start (a|an|new) (\w+ )?app/gi,
			/initialize (a|an|new) (\w+ )?app/gi,
			/scaffold (a|an|new) (\w+ )?app/gi,
			/setup (a|an|new) (\w+ )?app/gi,
			/create (a|an|new) (\w+ )?application/gi,
			/build (a|an|new) (\w+ )?application/gi,
			/develop (a|an|new) (\w+ )?application/gi,
			/create (a|an|new) (\w+ )?project/gi,
			/start (a|an|new) (\w+ )?project/gi,
			/initialize (a|an|new) (\w+ )?project/gi,
			/new (\w+ )?app/gi,
			/new (\w+ )?application/gi,
			/new (\w+ )?project/gi
		]

		// Check for app creation patterns
		for (const pattern of appCreationPatterns) {
			if (pattern.test(lowerTask)) {
				return true
			}
		}

		// Check for app-related keywords with creation verbs
		const creationVerbs = ['create', 'build', 'develop', 'make', 'start', 'initialize', 'setup', 'scaffold']
		const appKeywords = [
			'app', 'application', 'project', 'website', 'web app', 'mobile app',
			'desktop app', 'api', 'service', 'microservice', 'web service',
			'react app', 'vue app', 'angular app', 'nextjs app', 'nuxt app',
			'node app', 'express app', 'django app', 'flask app', 'rails app',
			'spring app', 'laravel app', 'mern app', 'mean app', 'jamstack'
		]
		...
		// Check for framework-specific app creation
		const frameworkPatterns = [
			/npx create-react-app/gi,
			/create-react-app/gi,
			/vue create/gi,
			/ng new/gi,
			/npx create-next-app/gi,
			/npx create-vite/gi,
			/expo init/gi,
			/flutter create/gi,
			/django-admin startproject/gi,
			/rails new/gi,
			/spring init/gi,
			/npm init/gi,
			/yarn create/gi,
	...
	private extractTechKeywords(task: string): string[] {
		const techKeywords: string[] = []

		// Common programming languages and frameworks
		const commonTechs = [
			'react', 'vue', 'angular', 'svelte', 'nextjs', 'nuxt', 'gatsby',
			'nodejs', 'express', 'fastify', 'koa', 'nestjs',
			'python', 'django', 'flask', 'fastapi', 'rails', 'laravel',
			'java', 'spring', 'springboot', 'maven', 'gradle',
			'javascript', 'typescript', 'css', 'html', 'sass', 'less',
			...
			'devops', 'terraform', 'ansible', 'puppet',
		...
		// Find mentions of technologies in the task
		...
		// Also look for specific patterns like "Create a X app" or "Build with Y"
		const techPatterns = [
			/create a (\w+) app/gi,
			/build with (\w+)/gi,
			/using (\w+)/gi,
		...
		for (const pattern of techPatterns) {
			...
			if (matches) {
				const capturedTech = matches[1] || matches[2]
				if (capturedTech && !techKeywords.includes(capturedTech.toLowerCase())) {
					techKeywords.push(capturedTech.toLowerCase())
				}
			}
		}

		return techKeywords.slice(0, 5) // Limit to top 5 most relevant
	}

	// Format search results into a readable context
	private formatSearchResults(
		codeContext: any,
		securityGuidelines: any,
		googleCodeWiki: any,
		owaspInfo: any
	): string {
		let context = "\n\n# 📚 Relevant Information for Your Project\n\n"

		// Add code context
		if (codeContext.results && codeContext.results.length > 0) {
			context += "## 📖 Code Context & Best Practices\n\n"
			codeContext.results.slice(0, 3).forEach((result: any, index: number) => {
		...
		if (securityGuidelines.results && securityGuidelines.results.length > 0) {
			context += "## 🔒 Security Guidelines\n\n"
			securityGuidelines.results.slice(0, 2).forEach((result: any, index: number) => {
				context += `### ${index + 1}. ${result.title}\n`
				context += `**Source:** ${result.url}\n`
				context += `**Content:** ${result.content.substring(0, 300)}...\n\n`
			})
		...
		// Add Google Code Wiki info
		if (googleCodeWiki.results && googleCodeWiki.results.length > 0) {
			context += "## 📖 Google Code Wiki Resources\n\n"
			googleCodeWiki.results.slice(0, 2).forEach((result: any, index: number) => {
				context += `### ${index + 1}. ${result.title}\n`
				context += `**Source:** ${result.url}\n`
				context += `**Content:** ${result.content.substring(0, 300)}...\n\n`
			})
		...
		// Add OWASP Top 10 info
		if (owaspInfo.results && owaspInfo.results.length > 0) {
			context += "## 🛡️ OWASP Top 10 Security Considerations\n\n"
			owaspInfo.results.forEach((result: any, index: number) => {
				context += `### ${index + 1}. ${result.title}\n`
				context += `**Source:** ${result.url}\n`
				context += `**Content:** ${result.content.substring(0, 300)}...\n\n`
			})
		...
		context += "---\n\n*This information was gathered using Tavily search to help you build a more secure and well-architected project. Consider these guidelines and best practices as you develop your solution.*\n"
	...
	private async startTask(task?: string, images?: string[], files?: string[]): Promise<void> {
		...
		// Display the user's task first
		await this.say("text", task, images, files)

		// Search for relevant information using Tavily if available and this is an app creation task
		let searchContext = ""
		if (task && tavilySearchService.hasValidApiKey() && this.isAppCreationTask(task)) {
			try {
				// Start with professional search status message
				await this.say("text", "🔍 Gathering development resources and security guidelines...")

				// Extract potential technologies from the task description
				const techKeywords = this.extractTechKeywords(task)

				// Perform all searches in parallel for efficiency
				const [codeContext, securityGuidelines, googleCodeWiki, owaspInfo] = await Promise.all([
					tavilySearchService.searchForCodeContext(task, techKeywords),
					tavilySearchService.searchForSecurityGuidelines(techKeywords),
					tavilySearchService.searchGoogleCodeWiki(techKeywords[0] || task),
					tavilySearchService.searchOWASPTop10()
				])

				// Format the search results
				searchContext = this.formatSearchResults(codeContext, securityGuidelines, googleCodeWiki, owaspInfo)

				// Professional completion message
				await this.say("text", "✅ Development resources loaded successfully")
			} catch (error) {
				console.error("Failed to search for relevant information:", error)
				await this.say("text", "⚠️ Unable to load development resources - proceeding with standard assistance")
	...
	async abortTask() {
		...
		// Analyze conversation for memory learning before aborting
		...
		// Dispose memory manager if available
	...
	async executeCommandTool(command: string): Promise<[boolean, ToolResponse]> {
		...
		// Detect shell and adapt command for the current environment
		let adaptedCommand = command
		try {
			const shellInfo = await ShellDetector.detectShell()
			Logger.info(`Detected shell: ${shellInfo.shell} on ${shellInfo.platform}`)
			
			// Adapt command for the detected shell
			adaptedCommand = await CommandAdapter.adaptCommand(command, shellInfo)
			
			if (adaptedCommand !== command) {
				Logger.info(`Command adapted for ${shellInfo.shell}: "${command}" -> "${adaptedCommand}"`)
			}
			
			// Check if command is safe to run
			const isSafe = await CommandAdapter.isCommandSafe(adaptedCommand, shellInfo)
			if (!isSafe) {
				Logger.warn(`Command flagged as potentially unsafe: ${adaptedCommand}`)
			}
		} catch (error) {
			Logger.error("Failed to detect shell or adapt command:", error)
			// Continue with original command if detection fails
		}

		Logger.info("Executing command in VS code terminal: " + adaptedCommand)

		const terminalInfo = await this.terminalManager.getOrCreateTerminal(cwd)
		terminalInfo.terminal.show() // weird visual bug when creating new terminals (even manually) where there's an empty space at the top.
		const process = this.terminalManager.runCommand(terminalInfo, adaptedCommand)
			...
			case "tool_use":
				const toolDescription = () => {
					...
					case "get_diagnostics":
						return `[${block.name} for '${block.params.file_path || 'workspace'}']`
					case "browser_action":
				...
				switch (block.name) {
					...
					case "get_diagnostics": {
						...
						const severity: string | undefined = block.params.severity
						const sharedMessageProps: ClineSayTool = {
							tool: "getDiagnostics",
							path: getReadablePath(cwd, removeClosingTag("file_path", filePath || "workspace")),
						}
						try {
							if (block.partial) {
								const partialMessage = JSON.stringify({
								...
								// Import and execute the diagnostics tool
								const { executeDiagnosticsTool } = await import("@core/tools/diagnosticsTool")
								const result = await executeDiagnosticsTool(cwd, filePath, severity)
								...
								if (this.shouldAutoApproveTool(block.name)) {
									...
									showNotificationForApprovalIfAutoApprovalEnabled(
										`Cline wants to get diagnostics for ${filePath || "workspace"}`,
									)
									this.removeLastPartialMessageIfExistsWithType("say", "tool")
									const didApprove = await askApproval("tool", completeMessage)
									if (!didApprove) {
							...
							await handleError("getting diagnostics", error)
							await this.saveCheckpoint()
							break
						}
					}
						...
						try {
							...
							if (block.partial) {
								...
								if (command) {
									...
									// Analyze conversation for memory learning
									await this.analyzeConversationForMemory()
								}
	...
	async loadContext(userContent: UserContent, includeFileDetails: boolean = false): Promise<[UserContent, string, boolean]> {
		...
		try {
			...
			if (contextInjectionEnabled && memoryEnabled && this.memoryManager) {
				// Extract task description from user content
				const taskDescription = processedUserContent
					.filter(block => block.type === 'text')
					.map(block => (block as any).text)
					.join(' ')
					.substring(0, 500) // Limit to first 500 chars for relevance matching
			...
			// Don't fail the task if memory injection fails
		}

		// Return all results with memory context appended to environment details
	...
	// Analyze conversation for memory learning when task completes
	private async analyzeConversationForMemory(): Promise<void> {
		try {
			// Check if memory manager is available
			if (!this.memoryManager) {
				console.log("[MEMORY] Memory manager not available for conversation analysis")
				return
			}

			console.log("[MEMORY] Task completed - analyzing conversation for memory learning")
			console.log(`[MEMORY] Total messages to analyze: ${this.clineMessages.length}`)
			console.log(`[MEMORY] Task ID: ${this.taskId}`)

			// Analyze the conversation for memory learning
			const result = await this.memoryManager.analyzeAndLearnFromConversation(this.clineMessages)
			console.log("[MEMORY] Conversation analysis completed for memory learning")
			console.log(`[MEMORY] Analysis result:`, result)

			// Notify user about memory learning
			await this.say("text", "🧠 Project details and patterns have been saved to memory for future reference.")

		} catch (error) {
			console.error("[MEMORY] Failed to analyze conversation for memory learning:", error)
			// Don't let memory analysis errors break the completion flow
		}
	}
}
