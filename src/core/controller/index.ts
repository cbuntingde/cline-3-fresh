...
import { initializeMemoryService } from "./memory/initializeMemoryService"
...
export class Controller {
	...
	*/
	...
	async handleWebviewMessage(message: WebviewMessage) {
		switch (message.type) {
			...
			case "updateSettings": {
				...
				// Tavily API key
				if (message.tavilyApiKey !== undefined) {
					await storeSecret(this.context, "tavilyApiKey", message.tavilyApiKey)
				}

				// Memory settings
				if (typeof message.memoryEnabled === "boolean") {
					await updateGlobalState(this.context, "memoryEnabled", message.memoryEnabled)
				}

				if (typeof message.autoLearnEnabled === "boolean") {
					await updateGlobalState(this.context, "autoLearnEnabled", message.autoLearnEnabled)
				}

				if (typeof message.contextInjectionEnabled === "boolean") {
					await updateGlobalState(this.context, "contextInjectionEnabled", message.contextInjectionEnabled)
				}

				// Diagnostics settings
				if (typeof message.diagnosticsEnabled === "boolean") {
					await updateGlobalState(this.context, "diagnosticsEnabled", message.diagnosticsEnabled)
				}

				if (message.diagnosticsSeverities) {
					await updateGlobalState(this.context, "diagnosticsSeverities", message.diagnosticsSeverities)
				}

				if (typeof message.diagnosticsMaxCount === "number") {
					await updateGlobalState(this.context, "diagnosticsMaxCount", message.diagnosticsMaxCount)
				}
			...
			case "grpc_response": {
				...
				console.log(
					"Received gRPC response in controller (should be handled by webview):",
					message.grpc_response?.request_id,
				)
				break
			}
	...
	async getStateToPostToWebview(): Promise<ExtensionState> {
		...
		const memoryEnabled = (await getGlobalState(this.context, "memoryEnabled")) as boolean | undefined
		const autoLearnEnabled = (await getGlobalState(this.context, "autoLearnEnabled")) as boolean | undefined
		const contextInjectionEnabled = (await getGlobalState(this.context, "contextInjectionEnabled")) as boolean | undefined
		const diagnosticsEnabled = (await getGlobalState(this.context, "diagnosticsEnabled")) as boolean | undefined
		const diagnosticsSeverities = (await getGlobalState(this.context, "diagnosticsSeverities")) as
			| ("error" | "warning" | "info" | "hint")[]
			| undefined
		const diagnosticsMaxCount = (await getGlobalState(this.context, "diagnosticsMaxCount")) as number | undefined
...
Commit message:`
	...
	async refreshDiagnostics() {
		try {
			...
			// Update the state with diagnostics summary
			const currentState = await this.getStateToPostToWebview()
			currentState.diagnosticsSummary = {
				errors,
				warnings,
				infos,
				hints,
			...
			console.log(`Diagnostics refreshed: ${errors} errors, ${warnings} warnings, ${infos} infos, ${hints} hints`)
		} catch (error) {
