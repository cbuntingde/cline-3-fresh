...
export class ServiceRegistry {
	...
	async handleRequest(controller: Controller, method: string, message: any): Promise<any> {
		let handler = this.methodRegistry[method]
		let pascalCaseMethod = ""

		// Try to find the method by converting camelCase to PascalCase if not found
		if (!handler) {
			pascalCaseMethod = this.camelToPascalCase(method)
			const pascalHandler = this.methodRegistry[pascalCaseMethod]

			if (pascalHandler) {
				// Use the PascalCase version
				handler = pascalHandler
				method = pascalCaseMethod
			} else {
				const isMethodStreaming = this.isStreamingMethod(method)
				const isPascalMethodStreaming = pascalCaseMethod ? this.isStreamingMethod(pascalCaseMethod) : false

				if (isMethodStreaming || isPascalMethodStreaming) {
	...
	async handleStreamingRequest(
		...
		let handler = this.streamingMethodRegistry[method]
		let pascalCaseMethod = ""

		// Try to find the method by converting camelCase to PascalCase if not found
		if (!handler) {
			pascalCaseMethod = this.camelToPascalCase(method)
			const pascalHandler = this.streamingMethodRegistry[pascalCaseMethod]
			...
			if (pascalHandler) {
				...
				const hasRegularMethod = !!this.methodRegistry[method]
				const hasPascalRegularMethod = pascalCaseMethod ? !!this.methodRegistry[pascalCaseMethod] : false

				if (hasRegularMethod || hasPascalRegularMethod) {
	...
	/**
	 * Convert camelCase method name to PascalCase
	 * @param method The camelCase method name
	 * @returns The PascalCase method name
	 */
	private camelToPascalCase(method: string): string {
