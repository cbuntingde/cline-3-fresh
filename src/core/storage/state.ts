...
export async function getAllExtensionState(context: vscode.ExtensionContext) {
	const [
		...
		// Memory settings
		getGlobalState(context, "memoryEnabled") as Promise<boolean | undefined>,
		getGlobalState(context, "autoLearnEnabled") as Promise<boolean | undefined>,
		getGlobalState(context, "contextInjectionEnabled") as Promise<boolean | undefined>,
		// Diagnostics settings
		getGlobalState(context, "diagnosticsEnabled") as Promise<boolean | undefined>,
		getGlobalState(context, "diagnosticsSeverities") as Promise<("error" | "warning" | "info" | "hint")[] | undefined>,
		getGlobalState(context, "diagnosticsMaxCount") as Promise<number | undefined>,
	])
	...
	if (storedApiProvider) {
		...
		if (apiKey) {
			...
			// New users should default to z.ai, since they've opted to use an API key instead of signing in
			apiProvider = "zai"
		}
	}
	...
	return {
		...
		autoLearnEnabled: autoLearnEnabled ?? true,
		contextInjectionEnabled: contextInjectionEnabled ?? true,
		// Diagnostics settings with defaults
		diagnosticsEnabled: diagnosticsEnabled ?? true,
		diagnosticsSeverities: diagnosticsSeverities ?? ["error", "warning"],
		diagnosticsMaxCount: diagnosticsMaxCount ?? 50,
	}
}
