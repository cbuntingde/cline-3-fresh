...
export class OpenAiHandler implements ApiHandler {
	...
	async *createMessage(systemPrompt: string, messages: Anthropic.Messages.MessageParam[]): ApiStream {
		...
		// Handle separate image model if configured
		if (this.options.openAiImageModelId) {
			const imageMessages = messages.filter((msg) => Array.isArray(msg.content) && msg.content.some((part) => part.type === "image"))
			if (imageMessages.length > 0) {
				...
				if (imageParts.length > 0) {
					const imageDescription = await this.client.chat.completions.create({
						model: this.options.openAiImageModelId,
						messages: [
							{
								role: "system",
					...
					const description = imageDescription.choices[0]?.message?.content || "No description available."

					// Replace images with description in the messages
